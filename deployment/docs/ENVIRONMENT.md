...
| `BACKEND_PORT` | Host port the backend is mapped to (container always listens on 8080) | `8090` | No |
| `API_BASE_URL` | Public base URL of the API (e.g. for links) | `http://localhost:8090` | Yes (production) |
...
| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `FLAG_ENCRYPTION_KEY` | 64 hex chars (32 bytes AES-256) for encrypting regex challenge flags | (none) | **Yes** (or from Vault) |
...
| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `JWT_ACCESS_SECRET` | Access token signing secret (min 32 chars) | (none) | **Yes** (or from Vault) |
| `JWT_REFRESH_SECRET` | Refresh token signing secret (min 32 chars) | (none) | **Yes** (or from Vault) |
| `JWT_DOWNLOAD_SECRET` | Download URL signing secret. If empty, derived via HMAC-SHA256 from `JWT_ACCESS_SECRET` | (derived) | No |
...
| `RESEND_API_KEY` | Resend API key | (none) | **Yes** if `RESEND_ENABLED=true` (or from Vault) |
...
| `STORAGE_S3_ENDPOINT` | S3 endpoint (host:port) | `seaweedfs:8333` | **Yes** if provider=s3 |
| `STORAGE_S3_PUBLIC_ENDPOINT` | Public S3 URL for presigned links | (empty) | Yes (production) |
| `STORAGE_S3_BUCKET` | S3 bucket name | `ctf` | **Yes** if provider=s3 |
| `STORAGE_S3_REGION` | S3 region | `us-east-1` | No |
| `STORAGE_S3_USE_SSL` | Use SSL for S3 | `false` | No |
| `STORAGE_S3_ACCESS_KEY` | S3 access key (direct, without Vault) | (none) | Yes if provider=s3 (or from Vault) |
| `STORAGE_S3_SECRET_KEY` | S3 secret key (direct, without Vault) | (none) | Yes if provider=s3 (or from Vault) |
...
| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `OAUTH_STATE_SECRET` | Secret for OAuth state parameter HMAC | (none) | **Yes** if any OAuth provider is enabled (or from Vault) |
...
| `GRAFANA_ADMIN_PASSWORD` | Grafana admin password | (none) | Yes (production) |
| `GRAFANA_PORT` | Host port Grafana is bound to | `3000` | No |
| `GF_SERVER_ROOT_URL` | Public root URL of Grafana (for OAuth redirects, links) | `https://grafana.ctfleague.ru` | Yes (production) |
...
| `SEAWEED_S3_ACCESS_KEY` | S3 access key written into `s3.json` and seeded into Vault | (none) | **Yes** |
| `SEAWEED_S3_SECRET_KEY` | S3 secret key written into `s3.json` and seeded into Vault | (none) | **Yes** |
...
> `run.sh` uses `SEAWEED_S3_ACCESS_KEY / SECRET_KEY` to generate `s3.json` (SeaweedFS identity) and to seed the same credentials into Vault at `astroctfb/storage`. The backend then reads them from Vault. These two variables must always be set; the `STORAGE_S3_ACCESS_KEY / SECRET_KEY` variables are a direct fallback only used when Vault is disabled.
...
Variables used to configure Alertmanager Telegram notifications. Not read by the backend.

| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `TELEGRAM_BOT_TOKEN` | Telegram bot token for alert notifications | (empty) | No (enables Telegram alerts if set) |
| `TELEGRAM_CHAT_ID` | Telegram chat ID for alert notifications | (empty) | No (required if `TELEGRAM_BOT_TOKEN` is set) |

> When both are set, `run.sh` generates `alertmanager.yml` with a Telegram receiver. When empty, a null-receiver is used (alerts are collected but not dispatched).
...
Variables written by `run.sh` into `.env` for its own use during reconfigure and nginx/alertmanager generation. Not read by the backend or Docker Compose.

| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `DOMAIN` | Base domain (e.g. `example.com`) used to derive all URLs | (none) | Yes (set by `run.sh` wizard) |
| `VAULT_ADMIN_IP` | Server public IP for the Vault nginx whitelist | (none) | Yes (set by `run.sh` wizard) |
