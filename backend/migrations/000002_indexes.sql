...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_submissions_ip_fail ON submissions (ip, created_at)
WHERE
    is_correct = FALSE;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_teams_scoreboard_filter ON teams (is_banned, is_hidden)
WHERE
    deleted_at IS NULL;
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_was_in_banned_team ON users (was_in_banned_team)
WHERE
    was_in_banned_team = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_submissions_banned_user_id ON submissions (banned_user_id)
WHERE
    banned_user_id IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_submissions_team_challenge_window ON submissions (
    team_id,
    challenge_id,
    created_at DESC
)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_solves_challenge_active ON solves (challenge_id, solved_at)
WHERE
    banned_team_id IS NULL
    AND banned_user_id IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_notifications_global_pinned_created ON notifications (
    is_global,
    is_pinned,
    created_at DESC
)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_notifications_unread ON user_notifications (user_id)
WHERE
    is_read = FALSE;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_solves_user_solved_at ON solves (user_id, solved_at DESC)
WHERE
    banned_team_id IS NULL
    AND banned_user_id IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_challenge_opens_team_challenge_opened ON challenge_opens (
    team_id,
    challenge_id,
    opened_at DESC
)
...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_backup_import_jobs_one_active ON backup_import_jobs ((1))
WHERE
    status IN ('queued', 'running');

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_challenges_next_challenge_id ON challenges (next_challenge_id)
WHERE
    next_challenge_id IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_challenge_topics_topic_id ON challenge_topics (topic_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ratings_challenge_active ON ratings (challenge_id, created_at)
WHERE
    banned_team_id IS NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_ban_appeals_one_pending_per_user ON ban_appeals (user_id)
WHERE
    decision = 'pending';
-- +goose Down
DROP INDEX CONCURRENTLY IF EXISTS idx_ban_appeals_one_pending_per_user;

DROP INDEX CONCURRENTLY IF EXISTS idx_ratings_challenge_active;
...
DROP INDEX CONCURRENTLY IF EXISTS idx_backup_import_jobs_one_active;
