...
func InitializeApp(cfg *config.Config, l logger.Logger, pool *pgxpool.Pool, redisClient *redis.Client, storageProvider storage.Provider, jwtService *jwt.JWTService, wsHub *websocket.Hub, mailer2 mailer.Mailer) (*App, error) {
	...
	challengeUseCase := ProvideChallengeUseCase(challengeRepo, tagRepo, solveRepo, txRepo, competitionRepo, teamRepo, redisClient, broadcaster, auditLogRepo, service)
	cache := ProvideCache(redisClient)
	solveUseCase := ProvideSolveUseCase(solveRepo, challengeRepo, competitionRepo, userRepo, teamRepo, txRepo, cache, broadcaster)
