...
const getAppSettingsForUpdate = `-- name: GetAppSettingsForUpdate :one
SELECT id, app_name, verify_emails, frontend_url, cors_origins,
       resend_enabled, resend_from_email, resend_from_name,
       verify_ttl_hours, reset_ttl_hours,
       submit_limit_per_user, submit_limit_duration_min,
...
func (q *Queries) GetAppSettingsForUpdate(ctx context.Context) (AppSettings, error) {
	row := q.db.QueryRow(ctx, getAppSettingsForUpdate)
	var i AppSettings
	err := row.Scan(
		&i.ID,
		&i.AppName,
...
const updateAppSettingsIfCurrent = `-- name: UpdateAppSettingsIfCurrent :one
UPDATE app_settings SET
    app_name = $1,
    verify_emails = $2,
    frontend_url = $3,
...
type UpdateAppSettingsIfCurrentParams struct {
	AppName                          string             `json:"app_name"`
	VerifyEmails                     bool               `json:"verify_emails"`
	FrontendURL                      string             `json:"frontend_url"`
	CORSOrigins                      string             `json:"cors_origins"`
...
func (q *Queries) UpdateAppSettingsIfCurrent(ctx context.Context, arg UpdateAppSettingsIfCurrentParams) (int32, error) {
	row := q.db.QueryRow(ctx, updateAppSettingsIfCurrent,
		arg.AppName,
		arg.VerifyEmails,
		arg.FrontendURL,
		arg.CORSOrigins,
