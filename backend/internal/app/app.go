...
func Run(cfg *config.Config, l *logger.Logger) {
	...
	challengeUC := usecase.NewChallengeUseCase(challengeRepo, solveRepo)
	solveUC := usecase.NewSolveUseCase(solveRepo)
	...
	go func() {
		l.Info("Starting HTTP server", nil, map[string]interface{}{"port": cfg.Port})
