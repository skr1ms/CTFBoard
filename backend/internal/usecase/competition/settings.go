...
func (uc *SettingsUseCase) validate(s *entity.AppSettings) error {
	if s.SubmitLimitPerUser < 1 {
		return fmt.Errorf("submit_limit_per_user must be >= 1")
	}
	if s.SubmitLimitDurationMin < 1 {
		return fmt.Errorf("submit_limit_duration_min must be >= 1")
	}
	if s.VerifyTTLHours < 1 || s.VerifyTTLHours > 168 {
		return fmt.Errorf("verify_ttl_hours must be between 1 and 168")
	}
	if s.ResetTTLHours < 1 || s.ResetTTLHours > 168 {
		return fmt.Errorf("reset_ttl_hours must be between 1 and 168")
	}
	...
	default:
		return fmt.Errorf("scoreboard_visible must be public, hidden, or admins_only")
	}
	return nil
}
