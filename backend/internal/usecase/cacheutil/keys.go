...
const (
	...
	// KeyScoreboardFrozen is the Redis key for the frozen scoreboard cache.
...
// KeyScoreboardFrozenAt returns the Redis cache key for a frozen scoreboard snapshot.
func KeyScoreboardFrozenAt(freezeUnix int64) string {
	return KeyScoreboardFrozenPrefix + strconv.FormatInt(freezeUnix, 10)
}

// KeyScoreboardBracketFrozenAt returns the Redis cache key for a bracket frozen scoreboard snapshot.
