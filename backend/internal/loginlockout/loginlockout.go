...
func (t *Tracker) IsLocked(ctx context.Context, email string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("Tracker - IsLocked: %w", err)
	}

	return n >= t.max, nil
}

// ClearFailed resets the failed login counter for the given email.
// Should be called on successful authentication to unblock the account immediately.
func (t *Tracker) ClearFailed(ctx context.Context, email string) error {
	...
	if err != nil {
		return fmt.Errorf("Tracker - ClearFailed: %w", err)
	}
...
func (t *Tracker) RecordFailed(ctx context.Context, email string) error {
	...
	if err != nil {
		return fmt.Errorf("Tracker - RecordFailed - Exec: %w", err)
	}

	durCmd, ok := cmds[1].(*redis.DurationCmd)
	if !ok {
		return fmt.Errorf("Tracker - RecordFailed: unexpected command type")
	}

	if err := durCmd.Err(); err != nil {
		return fmt.Errorf("Tracker - RecordFailed - TTL: %w", err)
	}

	ttl := durCmd.Val()
	if ttl <= 0 {
		if err := t.client.Expire(ctx, key, t.ttl).Err(); err != nil {
			return fmt.Errorf("Tracker - RecordFailed - Expire: %w", err)
		}
	}
