...
func NewUserRoutes(router chi.Router, userUC *usecase.UserUseCase, validator validator.Validator, logger logger.Interface, jwtService *jwt.JWTService) {
	...
	router.Route("/auth", func(r chi.Router) {
		...
		r.With(httpMiddleware.Auth(jwtService)).Get("/me", routes.Me)
	})
...
func (h *userRoutes) Register(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.Error("http - v1 - Register - Decode", err)
	...
	if err := h.validator.Validate(req); err != nil {
		h.logger.Error("http - v1 - Register - Validate", err)
		render.Status(r, http.StatusBadRequest)
		handleError(w, r, err)
		return
	}
	...
	if err != nil {
		h.logger.Error("http - v1 - Register - Register", err)
...
func (h *userRoutes) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.Error("http - v1 - Login - Decode", err)
		render.Status(r, http.StatusBadRequest)
		handleError(w, r, err)
		return
	}
	...
	if err := h.validator.Validate(req); err != nil {
		h.logger.Error("http - v1 - Login - Validate", err)
		render.Status(r, http.StatusBadRequest)
		handleError(w, r, err)
		return
	}
	...
	if err != nil {
		h.logger.Error("http - v1 - Login - Login", err)
...
func (h *userRoutes) Me(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("http - v1 - Me - GetByID", err)
		handleError(w, r, err)
		return
	}
...
func (h *userRoutes) GetProfile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("http - v1 - GetProfile - GetProfile", err)
		handleError(w, r, err)
		return
	}
