...
func (h *hintRoutes) GetByChallengeID(w http.ResponseWriter, r *http.Request) {
	...
	userId := r.Context().Value(httpMiddleware.UserIDKey).(string)
	user, err := h.userUC.GetByID(r.Context(), userId)
	if err != nil {
		h.logger.Error("http - v1 - GetByChallengeID - GetByID", err)
	...
	if err != nil {
		h.logger.Error("http - v1 - GetByChallengeID - GetByChallengeID", err)
		render.Status(r, http.StatusInternalServerError)
		handleError(w, r, err)
		return
	}
...
func (h *hintRoutes) UnlockHint(w http.ResponseWriter, r *http.Request) {
	...
	user, err := h.userUC.GetByID(r.Context(), userId)
	if err != nil {
		h.logger.Error("http - v1 - UnlockHint - GetByID", err)
		render.Status(r, http.StatusInternalServerError)
		handleError(w, r, err)
		return
	}
	...
	if err != nil {
		...
		h.logger.Error("http - v1 - UnlockHint - UnlockHint", err)
		render.Status(r, http.StatusInternalServerError)
		handleError(w, r, err)
		return
	}
